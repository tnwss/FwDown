	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				if ( isset( $value['before_row'] ) ) {
						$output .= $value['before_row'];
				}
			...
			// accordion
			case 'accordion':
			   $uploader = Options_Framework_Media_Uploader::optionsframework_uploader( $value['id'], $val, null );
			   $output .= '<div class="accordion-contari">'.$uploader.'<input id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '"' . $placeholder . ' /></div>';
				break;
